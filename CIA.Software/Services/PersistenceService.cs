...
namespace CIA.HelpDesk.Services
{
    public class PersistenceService
    {
        ...
        public CentralRepository LoadCentral()
        {
            var p = PathFor("central");
            if (!File.Exists(p)) return new CentralRepository();
            var json = File.ReadAllText(p);
            return JsonSerializer.Deserialize<CentralRepository>(json, _options) ?? new CentralRepository();
        }

        public void SaveCentral(CentralRepository central)
        {
            var p = PathFor("central");
            var json = JsonSerializer.Serialize(central, _options);
            File.WriteAllText(p, json);
        }
    }
}
